...
public class UserDaoImpl extends GenericDaoImpl<User> implements UserDao {
	...
	public User findBySoldItemInitialPrice(String email, String boughtItem, BigDecimal initialPrice) {
		Query query=entityManager.createQuery("select u from User u, Item i where"
				+ " i.name = :boughtItem and i.initialPrice > :initialPrice");
  return (User) query.setParameter("boughtItem", boughtItem).setParameter("initialPrice", initialPrice).setFirstResult(1).getSingleResult();		     
